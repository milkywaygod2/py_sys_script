...
def create_word_from_array(text_array: List[str], output_file: str,
    ...
    """
    Create Word document from array of strings.
    문자열 배열에서 Word 문서를 생성합니다.
    
    Args:
        text_array: Array of text strings (each becomes a paragraph)
                    텍스트 문자열 배열 (각각 단락이 됨)
...
def add_heading_to_word(file_path: str, heading_text: str, level: int = 1) -> bool:
    """
    Add a heading to existing Word document.
    기존 Word 문서에 제목을 추가합니다.
...
def add_image_to_word(file_path: str, image_path: str, 
    ...
    Args:
        ...
        image_path: Path to image file
                    이미지 파일 경로
        width_inches: Image width in inches (None for original size)
                      인치 단위 이미지 너비 (None이면 원본 크기)
...
def read_word_document(file_path: str) -> Optional[List[str]]:
    """
    Read all paragraphs from Word document.
    Word 문서에서 모든 단락을 읽습니다.
    ...
    Returns:
        List of paragraph texts or None if error
        단락 텍스트 리스트, 에러시 None
    """
    try:
        from docx import Document
...
def merge_word_documents(input_files: List[str], output_file: str) -> bool:
    ...
    여러 Word 문서를 하나로 병합합니다.
    
    Args:
        input_files: List of Word document paths
                     Word 문서 경로 리스트
        output_file: Output Word file path
                     출력 Word 파일 경로
...
def get_word_document_info(file_path: str) -> Optional[Dict[str, Any]]:
    """
    Get information about Word document.
    Word 문서에 대한 정보를 가져옵니다.
    ...
    Returns:
        Dictionary with document information or None if error
        문서 정보 딕셔너리, 에러시 None
    """
    try:
        from docx import Document
...
def create_word_template(output_file: str, placeholders: List[str]) -> bool:
    """
    Create a Word document template with placeholders.
    자리 표시자가 있는 Word 문서 템플릿을 생성합니다.
