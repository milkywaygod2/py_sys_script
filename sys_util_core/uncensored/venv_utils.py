...
def install_package(venv_path: str, package_name: str, 
    ...
    try:
        ...
        if cmd_ret[0] != 0:
            raise Exception(cmd_ret[1])        
        return True, f"Package installed successfully: {cmd_ret[1]}"
    except Exception as e:
...
def uninstall_package(venv_path: str, package_name: str, 
    ...
    try:
        ...
        if cmd_ret[0] != 0:
            raise Exception(cmd_ret[1])        
        return True, f"Package uninstalled successfully: {cmd_ret[1]}"
    except Exception as e:
...
def list_packages(venv_path: str, format: str = 'columns') -> Tuple[bool, str, List[Dict[str, str]]]:
    try:
        ...
        if cmd_ret[0] != 0:
            raise Exception(cmd_ret[1])
        if format == 'json':
            packages = json.loads(cmd_ret[1])        
        return True, cmd_ret[1], packages
    except Exception as e:
...
def upgrade_pip(venv_path: str) -> Tuple[bool, str]:
    try:
        ...
        return True, f"pip upgraded successfully: {cmd_ret[1]}"
    except Exception as e:
...
def install_requirements(venv_path: str, requirements_file: str) -> Tuple[bool, str]:
    try:
        ...
        if cmd_ret[0] != 0:
            raise Exception(cmd_ret[1])        
        return True, f"Requirements installed successfully: {cmd_ret[1]}"
    except Exception as e:
...
def ensure_pyinstaller(venv_path: str, version: Optional[str] = None) -> Tuple[bool, str]:
    try:
        ...
        if cmd_ret[0] != 0:
            raise Exception(cmd_ret[1])
        return True, f"PyInstaller ensured in virtual environment: {cmd_ret[1]}"
    except Exception as e:
